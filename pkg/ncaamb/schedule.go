...
var scheduleURLTemplate = "https://api.sportradar.us/ncaamb/production/v7/en/games/%d/%s/schedule.json?api_key=%s"
...
func FetchSchedule(c http.Client, t time.Time, st sr.SeasonType, apiKey string) (Schedule, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return Schedule{}, fmt.Errorf("invalid response code from sportradar ncaamb schedule request: %s %d", url, resp.StatusCode)
	}
